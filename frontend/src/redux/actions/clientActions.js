...
export const getAllClientsInfo = () => async (dispatch, getState) => {
  try {
    dispatch({
      type: CLIENT_ALL_GET_REQUEST,
    })
    ...
    const { data } = await axios.get(`/api/client`, config)

    dispatch({
      type: CLIENT_ALL_GET_SUCCESS,
      payload: data,
    })
  } catch (error) {
    dispatch({
      type: CLIENT_ALL_GET_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
